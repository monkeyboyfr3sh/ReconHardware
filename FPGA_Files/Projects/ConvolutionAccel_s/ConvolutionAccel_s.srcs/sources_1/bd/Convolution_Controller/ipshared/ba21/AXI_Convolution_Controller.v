...
// State flags
reg RDst, MULTIst;
reg [$clog2(KERNEL_SIZE)-1:0] newline_cnt;
...
line_buffer(
    ...
    .rst(~axi_reset_n),
    .data_in(s_axis_data),
    .add_r(current_x + KERNEL_SIZE-newline_cnt),
    .wr_en(lb_wr_en_comb),
    .r_en(lb_r_en),
...
always @(posedge axi_clk) begin
    ...
    //cStart triggers matrixcontroller to start
        ...
        if(RDst)begin
            ...
            // Continue reading data into line buffer until full, if valid is high, should not be ready
            if(!lb_full)begin
