...
always @(posedge axi_clk) begin
    ...
    if(!axi_reset_n||RSTst)begin
        ...
        if(m_axis_rx_stat)begin
            RDst <= 0; POOLst <= 0; IDst <= !(avg_buff_cnt==0);
            m_axis_valid <= 0;
            m_axis_data <= 0;
        ...
        if(avg_buff_cnt>0)begin
            ...
            if(avg_buff_cnt==1)begin
                if(current_y+KERNEL_SIZE >= image_height && current_x+KERNEL_SIZE >= image_width)begin
                    lb_force_rst <= 1;
                    current_y <= 0;
                    m_axis_last <= 1;
                    lb_force_rst <= 0;
    ...
    // cStart triggers matrixcontroller to start
        ...
        //In a pool state, dataSet should be filled with needed values
        if(POOLst) begin
